...
public final class Utils {
    ...
    public static void addCustomTransforms(final List<Transform> transforms, final Properties xParams, final String xmlSignaturePrefix) {
        ...
        for (int i = 0; i < numTransforms; i++) {
            ...
            if (Transform.XPATH.equals(transformType) && transformBody != null) {
                try {
                    transformParam =
                        new XPathFilterParameterSpec(transformBody, Collections.singletonMap(xmlSignaturePrefix, XMLSignature.XMLNS));
                } catch (final Exception e) {
                    logger.warning("No se han podido crear los parametros para una transformacion XPATH, se omitira: " + e);
                    continue;
                }
                ...
                if ("subtract".equals(transformSubtype)) {
                    ...
                    logger.warning("Se ha solicitado aplicar una transformacion XPATH2 de un tipo no soportado: " + transformSubtype);
                    continue;
                }
                try {
                    transformParam =
                        new XPathFilter2ParameterSpec(Collections.singletonList(new XPathType(transformBody, xPath2TransformFilter)));
                } catch (final Exception e) {
                    logger.warning("No se han podido crear los parametros para una transformacion XPATH2, se omitira: " + e);
                    continue;
                }
    ...
    public static List<Transform> getObjectReferenceTransforms(final Node referenceNode, final String namespacePrefix)
        ...
        for (int i = 0; i < tmpNodeList.getLength(); i++) {
            ...
            if (
                "Transforms".equals(tmpNodeList.item(i).getNodeName()) ||
     ...
     * XPATH2.
     */
    private static TransformParameterSpec getTransformParameterSpec(final Node transformNode, final String namespacePrefix)
        ...
        if (algorithm != null && (Transform.XPATH.equals(algorithm) || Transform.XPATH2.equals(algorithm))) {
            ...
            if (Transform.XPATH.equals(algorithm)) {
                ...
                for (int i = 0; i < xpathTransforms.getLength(); i++) {
                    ...
                    // namespace indicado
                    if (
                        "XPath".equals(xpathTransformNode.getNodeName()) ||
     ...
     * si es XMLDSig
     */
    public static void checkIllegalParams(
        final String format,
        final String mode,
        final URI uri,
        ...
        if (xades) { // XAdES
            if (format.equals(SignConstants.SIGN_FORMAT_XADES_ENVELOPED) && mode.equals(SignConstants.SIGN_MODE_EXPLICIT)) {
                throw new UnsupportedOperationException("El formato Enveloped es incompatible con el modo de firma explicito");
            }
            if (
                !format.equals(SignConstants.SIGN_FORMAT_XADES_DETACHED) &&
            ...
            if (format.equals(SignConstants.SIGN_FORMAT_XMLDSIG_ENVELOPED) && mode.equals(SignConstants.SIGN_MODE_EXPLICIT)) {
                throw new UnsupportedOperationException("No se puede realizar una firma enveloped sobre un contenido explicito");
            }
            if (format.equals(SignConstants.SIGN_FORMAT_XMLDSIG_EXTERNALLY_DETACHED) && mode.equals(SignConstants.SIGN_MODE_IMPLICIT)) {
            ...
            if (
                !format.equals(SignConstants.SIGN_FORMAT_XMLDSIG_DETACHED) &&
    ...
    public static byte[] writeXML(Node node, Map<String, String> props) {
        ...
        try {
            ...
            logger.warning("La codificacion '" + xmlEncoding + "' no es valida, se usara la por defecto del sistema: " + e);
    ...
    public static SignInfo getSimpleSignInfoNode(String namespace, Element signature) {
            ...
            try {
                signingTime =
                    new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss")
                        .parse(((Element) signature.getElementsByTagNameNS(namespace, "SigningTime").item(0)).getTextContent());
            } catch (Exception e) {
        ...
        ssi.setSignAlgorithm(
            ((Element) signature.getElementsByTagNameNS(XMLConstants.DSIGNNS, "SignatureMethod").item(0)).getAttribute("Algorithm")
        );
    ...
    public static String getStringInfoNode(final Element signature) {
        return Util.getCN(Utils.getCertificate(signature.getElementsByTagNameNS(XMLConstants.DSIGNNS, "X509Certificate").item(0)));
    }
    ...
    public static String getCounterSignerReferenceId(final Element signature, final NodeList signatureValues) {
        ...
        final String uri =
            ((Element) signature.getElementsByTagNameNS(XMLConstants.DSIGNNS, "Reference").item(0)).getAttribute("URI") //$NON-NLS-1$
                .substring(1); //$NON-NLS-1$
