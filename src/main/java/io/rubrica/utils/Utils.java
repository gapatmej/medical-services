...
public class Utils {
    ...
    public static String getUID(final String principal) {
        ...
        if (i != -1) {
            logger.warning("No se ha podido obtener el UID, se devolvera el fragmento mas significativo");
    ...
    public static Date getSignTime(String fechaHora) {
        ...
        try {
            ...
            logger.severe("La fecha indicada ('" + fechaHora + "') como momento de firma para PDF no sigue el patron ISO-8601: " + e);
            return new Date();
        }
    }

    public static List<Certificado> datosP7mToCertificado(List<X509Certificate> certificados, List<Date> fechasFirmados)
        throws RubricaException, HoraServidorException, IOException, CertificadoInvalidoException, EntidadCertificadoraNoValidaException, ConexionValidarCRLException, CRLValidationException, OcspValidationException {
        ...
        if (signInfos == null || signInfos.isEmpty()) {
            ...
            for (SignInfo signInfo : signInfos) {
                ...
                try {
                    ...
                    List<String> signatureNames = pdfReader.getAcroFields().getSignatureNames();
                    for (String signatureName : signatureNames) {
                        if (
                            signInfo
                                .getSigningTime()
                                .equals(pdfReader.getAcroFields().verifySignature(signatureName).getSignDate().getTime())
                        ) {
    ...
    //    }
    public static List<Certificado> signInfosToCertificados(List<SignInfo> signInfos)
        throws DocumentoException, CertificadoInvalidoException, IOException {
        List<Certificado> certificados = new ArrayList<>();
        if (signInfos == null || signInfos.isEmpty()) {
            throw new DocumentoException("Documento sin firmas");
        } else {
    ...
    public static List<Certificado> verificarDocumento(File documento)
        throws IOException, KeyStoreException, OcspValidationException, SignatureException, RubricaException, ConexionInvalidaOCSPException, HoraServidorException, CertificadoInvalidoException, EntidadCertificadoraNoValidaException, ConexionValidarCRLException, SignatureVerificationException, DocumentoException, CRLValidationException, Exception {
    ...
    public static String leerXmlSRI(File documento) {
        ...
        try {
            ...
            if (xml.contains(inicioTexto1)) { //si la línea contiene el texto buscado
                xml = xml.substring(xml.lastIndexOf(inicioTexto1) + inicioTexto1.length(), xml.indexOf(finTexto1));
            } else { //si la línea contiene el texto buscado
                xml = xml.substring(xml.lastIndexOf(inicioTexto2) + inicioTexto2.length(), xml.indexOf(finTexto2));
            }
        } catch (Exception e) {
