...
public class PDFSigner implements Signer {
    ...
    public byte[] sign(byte[] data, String algorithm, PrivateKey key, Certificate[] certChain, Properties xParams)
        ...
        try {
            ...
            logger.warning(
                "Se ha indicado un tamaño de letra invalida ('" +
                extraParams.getProperty(FONT_SIZE) +
                "'), se usara el tamaño por defecto: " +
                fontSize +
                " " +
                e
            );
        ...
        try {
            ...
            logger.warning(
                "Se ha indicado un numero de pagina invalido ('" +
                extraParams.getProperty(LAST_PAGE) +
                "'), se usara la ultima pagina: " +
                e
            );
        }
        ...
        try {
            ...
            throw new RubricaException(
                io.rubrica.utils.PropertiesUtils.getMessages().getProperty("mensaje.error.driver_problemas") + "\n",
                ec
            );
    ...
    public List<SignInfo> getSigners(byte[] sign) throws InvalidFormatException, IOException {
        ...
        try {
            ...
            logger.severe("No se ha podido obtener la informacion de los firmantes del PDF, se devolvera un arbol vacio: " + e);
        ...
        for (String signatureName : names) {
            ...
            if (
                PDFNAME_ETSI_RFC3161.equals(pdfDictionary.get(PdfName.SUBFILTER)) ||
            ...
            try {
                ...
                e.printStackTrace();
                logger.severe(
                    "El PDF contiene una firma corrupta o con un formato desconocido (" +
                    signatureName +
                    "), se continua con las siguientes si las hubiese: " +
                    e
                );
                continue;
            }
            ...
            try {
                ...
                e.printStackTrace();
                logger.severe("No se ha podido obtener informacion de una de las firmas del PDF, se continuara con la siguiente: " + e);
                continue;
            }
