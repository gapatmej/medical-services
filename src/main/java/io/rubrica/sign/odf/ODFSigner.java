...
public class ODFSigner implements Signer {
    ...
    public byte[] sign(byte[] data, String algorithm, PrivateKey key, Certificate[] certChain, Properties extraParams)
        ...
        if (!SIGN_ALGORITHM_SHA1WITHRSA.equals(algorithm)) {
            logger.warning(
                "Se ha indicado '" + algorithm + "' como algoritmo de firma, pero se usara 'SHA1withRSA' por necesidades del formato ODF"
            );
        }
        ...
        try {
            ...
            try (ZipFile zf = new ZipFile(zipFile)) {
                ...
                try {
                    ...
                    throw new RubricaException(
                        "No se ha podido obtener un generador de huellas digitales con el algoritmo: " + DEFAULT_DIGEST_METHOD,
                        e
                    );
                }
                ...
                fac
                    ...
                    .sign(new DOMSignContext(key, rootSignatures));
    ...
    public List<SignInfo> getSigners(byte[] sign) throws InvalidFormatException, IOException {
        ...
        try {
            ...
            try (ZipFile zf = new ZipFile(zipFile); InputStream signIs = zf.getInputStream(zf.getEntry(SIGNATURES_PATH))) {
        ...
        try {
            signingTime =
                new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss")
                    .parse(
                        ((Element) signature.getElementsByTagNameNS("http://purl.org/dc/elements/1.1/", "date").item(0)).getTextContent()
                    );
        } catch (Exception e) {
        ...
        try {
            pkcs1 =
                Base64
                    .getDecoder()
                    .decode(
                        ((Element) signature.getElementsByTagNameNS(XMLConstants.DSIGNNS, "SignatureValue").item(0)).getTextContent()
                            .trim()
                            .replace("\r", "")
                            .replace("\n", "")
                            .replace(" ", "")
