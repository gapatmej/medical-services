...
public final class XAdESUtil {
    ...
    static boolean checkSignNodes(List<Node> signNodes) {
        for (Node signNode : signNodes) {
            ...
            for (final String xadesNamespace : SUPPORTED_XADES_NAMESPACE_URIS) {
                lenCount = lenCount + ((Element) signNode).getElementsByTagNameNS(xadesNamespace, "QualifyingProperties").getLength();
            }
            if (lenCount == 0) {
                return false;
            }
    ...
    static RubricaXMLAdvancedSignature getXmlAdvancedSignature(
        XAdES_EPES xades,
    ...
    static Element getFirstElmentFromXPath(String xpathExpression, Element sourceElement) throws RubricaException {
        ...
        try {
            ...
            throw new RubricaException(
                "No se ha podido evaluar la expresion indicada para la insercion de la firma Enveloped ('" + xpathExpression + "'): " + e1,
                e1
            );
        }
        if (nodeList.getLength() < 1) {
            throw new RubricaException(
                "La expresion indicada para la insercion de la firma Enveloped ('" + xpathExpression + "') no ha devuelto ningun nodo"
            );
        }
        if (nodeList.getLength() > 1) {
            logger.warning(
                "La expresion indicada para la insercion de la firma Enveloped ('" +
                xpathExpression +
     ...
     * XAdES.
     */
    public static List<CommitmentTypeIndication> parseCommitmentTypeIndications(Properties xParams, String signedDataId) {
        ...
        try {
            ...
            logger.severe(
                ...
                tmpStr +
                "), no se anadira el CommitmentTypeIndication: " +
                e
            );
            return ret;
        }
        ...
        for (int i = 0; i <= nCtis; i++) {
            ...
            tmpStr =
                xParams.getProperty(
                    XAdESExtraParams.COMMITMENT_TYPE_INDICATION_PREFIX +
                    Integer.toString(i) +
            ...
            if (identifier == null) {
                logger.severe(
                    "El identificador del CommitmentTypeIndication " +
                    i +
                    " no es un tipo soportado (" +
                    tmpStr +
                    "), se omitira y se continuara con el siguiente"
                );
                continue;
            }
            ...
            ret.add(
                new CommitmentTypeIndicationImpl(
                    ...
                    signedDataId != null
                        ? "#" + signedDataId // Una URI, pero se acepta null
                        : null, //$NON-NLS-1$
                    commitmentTypeQualifiers // Lista de elementos
                    // textuales (opcional)
                )
            );
        }
    ...
    static Element getRootElement(Document docSignature, Properties extraParams) {
        ...
        afirmaRoot.setAttributeNS(null, FirmadorXAdES.ID_IDENTIFIER, nodeName + "-Root-" + UUID.randomUUID().toString());

        return afirmaRoot;
    }

    static List<Reference> createManifest(
        final List<Reference> referenceList,
        final XMLSignatureFactory fac,
        final RubricaXMLAdvancedSignature xmlSignature,
        ...
        referenceList.add(
            fac.newReference(
                "#" + manifestId,
                digestMethod,
                canonicalizationTransform != null ? Collections.singletonList(canonicalizationTransform) : new ArrayList<Transform>(0),
                XAdESSigner.MANIFESTURI,
