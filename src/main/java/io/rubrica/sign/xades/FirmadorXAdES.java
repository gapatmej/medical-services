...
import es.uji.crypto.xades.jxades.security.xml.XAdES.*;
...
public final class FirmadorXAdES {
    ...
    public static byte[] sign(byte[] data, String algorithm, PrivateKey pk, Certificate[] certChain, Properties xParams)
        ...
        boolean avoidXpathExtraTransformsOnEnveloped = Boolean.parseBoolean(
            extraParams.getProperty(XAdESExtraParams.AVOID_XPATH_EXTRA_TRANSFORMS_ON_ENVELOPED, Boolean.FALSE.toString())
        );
        ...
        String envelopedNodeXPath = extraParams.getProperty(XAdESExtraParams.INSERT_ENVELOPED_SIGNATURE_ON_NODE_BY_XPATH);

        String nodeToSign = extraParams.getProperty(XAdESExtraParams.NODE_TOSIGN);

        String digestMethodAlgorithm = extraParams.getProperty(XAdESExtraParams.REFERENCES_DIGEST_METHOD, XAdESSigner.DIGEST_METHOD);

        String canonicalizationAlgorithm = extraParams.getProperty(
            XAdESExtraParams.CANONICALIZATION_ALGORITHM,
        ...
        String signedPropertiesTypeUrl = extraParams.getProperty(
            XAdESExtraParams.SIGNED_PROPERTIES_TYPE_URL,
            XAdESSigner.XADES_SIGNED_PROPERTIES_TYPE
        );

        boolean ignoreStyleSheets = Boolean.parseBoolean(
            extraParams.getProperty(XAdESExtraParams.IGNORE_STYLE_SHEETS, Boolean.FALSE.toString())
        );
        ...
        boolean headless = Boolean.parseBoolean(extraParams.getProperty(XAdESExtraParams.HEADLESS, Boolean.TRUE.toString()));
        ...
        boolean facturaeSign = Boolean.parseBoolean(extraParams.getProperty(XAdESExtraParams.FACTURAE_SIGN, Boolean.FALSE.toString()));
        ...
        try {
            uri =
                extraParams.getProperty(XAdESExtraParams.URI) != null
        ...
        try {
            ...
            throw new RubricaException(
                "No se ha podido obtener un generador de huellas digitales para el algoritmo '" + digestMethodAlgorithm + "'",
                e
            );
        }
        ...
        if (canonicalizationAlgorithm != null) {
            try {
                ...
                throw new RubricaException(
                    "No se ha posido crear el canonizador para el algoritmo indicado (" + canonicalizationAlgorithm + "): " + e1,
                    e1
                );
            }
        } else {
        ...
        try {
            ...
            // Crea la referencia
            referenceList.add(
                fac.newReference(nodeToSign != null ? "#" + nodeToSign : "", digestMethod, transformList, XMLConstants.OBJURI, referenceId)
            );
        } catch (Exception e) {
        ...
        XAdES_EPES xades = (XAdES_EPES) XAdES.newInstance(
            ...
            signatureInsertionNode != null
                ? // firma (como hijo), si no // Nodo donde se inserta la
        ...
        ObjectIdentifierImpl objectIdentifier = oid != null
            ? new ObjectIdentifierImpl("OIDAsURN", (oid.startsWith("urn:oid:") ? "" : "urn:oid:") + oid, null, new ArrayList<String>(0))
            : null;
        ...
        RubricaXMLAdvancedSignature xmlSignature = XAdESUtil.getXmlAdvancedSignature(
            xades,
        ...
        try {
            ...
            throw new UnsupportedOperationException(
                "Los formatos de firma XML no soportan el algoritmo de firma '" + algorithm + "':" + e,
                e
            );
        } catch (final Exception e) {
