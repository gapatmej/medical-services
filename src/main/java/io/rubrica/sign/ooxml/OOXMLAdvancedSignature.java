...
final class OOXMLAdvancedSignature extends XMLAdvancedSignature {
    ...
    void sign(X509Certificate[] certChain, PrivateKey privateKey, String signatureMethod, List<?> refsIdList, String signatureIdPrefix)
        throws MarshalException, GeneralSecurityException, XMLSignatureException {
        List<?> referencesIdList = new ArrayList<>(refsIdList);

        if (
            WrappedKeyStorePlace.SIGNING_CERTIFICATE_PROPERTY.equals(getWrappedKeyStorePlace()) && certChain != null && certChain.length > 0
        ) {
        ...
        addXMLObject(
            marshalXMLSignature(
                ...
                Arrays.asList( // En OOXML las SignedProperties se canonicalizan
                    Utils.getDOMFactory().newTransform("http://www.w3.org/TR/2001/REC-xml-c14n-20010315", (TransformParameterSpec) null)
                )
            )
        );
